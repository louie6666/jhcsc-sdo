...
<style>
    ...
    div .equipment-container .stats-text {
    ...
    .equipment-container .header-actions {
        display: flex;
        gap: 0.75rem;
        align-items: center;
    }
    ...
    .csv-tooltip-container .tooltip-text {
        ...
        z-index: 100;
        top: 125%;
        /* Position dynamically below the button */
        left: 50%;
        transform: translateX(-50%);
        /* Centered perfectly dynamically */
        opacity: 0;
...
<div class="equipment-container">
    <header class="inventory-header">
        <p class="stats-text">
            There are a total of <b><?php echo number_format($physical_sum); ?></b> physical items recorded across
            <b><?php echo $total_categories; ?></b> distinct categories.
        </p>
        <div class="header-actions">
            ...
            <div class="saas-dropdown-wrapper">
                <button
                    class="btn-filter <?php echo isset($_GET['filter']) && $_GET['filter'] !== '' ? 'active-filter' : ''; ?>"
                    onclick="toggleAppDropdown('filter-menu')">
                    <span class="material-symbols-outlined icon-18">filter_list</span>
                    <?php
                    $fLabel = "Filter";
                    if (isset($_GET['filter'])) {
                        if ($_GET['filter'] == 'in_demand')
                            $fLabel = 'In Demand';
                        if ($_GET['filter'] == 'low_stock')
                            $fLabel = 'Low Stock';
                        if ($_GET['filter'] == 'out_of_stock')
                            $fLabel = 'Out of Stock';
                    }
                    echo $fLabel;
                    ?>
                ...
                <div class="saas-dropdown-menu" id="filter-menu">
                    ...
                    <div class="saas-dropdown-item" onclick="applyGridFilter('filter', 'out_of_stock')">Out of Stock
                    </div>
                </div>
            </div>
            ...
            <div class="saas-dropdown-wrapper">
                <button
                    class="btn-filter <?php echo isset($_GET['category']) && $_GET['category'] !== '' ? 'active-filter' : ''; ?>"
                    onclick="toggleAppDropdown('category-menu')">
                ...
                <div class="saas-dropdown-menu" id="category-menu" style="max-height: 300px; overflow-y: auto;">
                    ...
                    <?php foreach ($all_categories as $cat): ?>
                        <div class="saas-dropdown-item"
                            onclick="applyGridFilter('category', '<?php echo $cat['category_id']; ?>')">
                ...
                <div class="card-body">
                    ...
                    <div class="card-footer">
                        <button class="btn-action"
                            onclick="openStockModal(<?php echo $row['equipment_id']; ?>, '<?php echo addslashes($row['name']); ?>', <?php echo $row['total_qty']; ?>)">
                            <span class="material-symbols-outlined icon-16">add</span> Stock
                        </button>
                        <button class="btn-action btn-action-small"
                            onclick="openEditModal(<?php echo $row['equipment_id']; ?>, '<?php echo addslashes($row['name']); ?>', '<?php echo addslashes($row['storage_location'] ?? ''); ?>', '<?php echo addslashes($row['image_url'] ?? ''); ?>')">
    ...
    <?php if ($unique_types > ($limit - 1)): ?>
        <div class="saas-pagination">
            <a href="#"
                onclick="loadModule('modules/equipments/equipment.php?page=<?php echo max(1, $page - 1) . $filter_params; ?>'); return false;"
                class="saas-nav-btn <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
            ...
            <div class="saas-pages">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="#"
                        onclick="loadModule('modules/equipments/equipment.php?page=<?php echo $i . $filter_params; ?>'); return false;"
                        class="saas-page-link <?php echo ($i == $page) ? 'active' : ''; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>
            </div>
            ...
            <a href="#"
                onclick="loadModule('modules/equipments/equipment.php?page=<?php echo min($total_pages, $page + 1) . $filter_params; ?>'); return false;"
                class="saas-nav-btn <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>">
